...
connection consists of the following minimum three parts:

1. `rpc` - used to make [Solana RPC](https://solana.com/docs/rpc) requests over HTTP (the most
   common way)
...
For local scripts, JavaScript server runtimes (like NodeJS and Bun), and backend applications its
common to load a keypair file from either a file or an ENV variable. The following is how to
accomplish this for both libraries:
