...
description: Create a client connection to the Solana blockchain
---

Setting up a client connecting to the Solana blockchain is a very important part of your
application. This client connection is how your application will be sending and receiving data from
the Solana JSON RPC layer, including [fetching accounts](#todo) and [sending transactions](#todo).
...
- using the [`createSolanaClient()`](#create-a-solana-rpc-connection) function (recommended in order
  to reduce application boilerplate)
- manually initialize them all individually

## Create a Solana RPC connection

Create the Solana `rpc` and `rpcSubscriptions` connections from any RPC URL or standard Solana
...
programs. The structure of an account's data and associated metadata is called an `AccountInfo`.

To get the from your RPC:
...
<Callout title="" type="error">
  ...
  within the account itself. This is fine for accounts with small bits of data stored in them, but
  fetching accounts with larger amounts of `data` will result in an error with the `base58`
  encoding.
</Callout>

It is strongly recommended to utilize the `base64` encoding when fetching account's from the
blockchain:
...
An even better solution is to utilize the `fetchEncodedAccount()` function to fetch accounts:
