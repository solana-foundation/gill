...
transaction and manually inserts the reference key as a non-signer on the
...
[monitor for the reference key](#monitoring-for-a-reference-key) and trigger their business logic.
...
instructions from these programs. The following programs are know to fall into this condition and
...
Using gill's `getOldestSignatureForAddress()` function, we can fetch the oldest signature that
includes the reference key. If the reference key is not found, it will throw an error.
...
try {
  const oldestSignature = await getOldestSignatureForAddress(rpc, reference);

  const transaction = await rpc.getTransaction(oldest.signature).send();
...
Your application can then fetch and process the transaction with the signature in `oldestSignature`,
validating the transaction is as expected.

After your application validates the oldest transaction accomplishes the expected on-chain actions
(e.g. correct token balance changes, etc), you can proceed with any of your success business logic.

<Callout type="warn" title="Reference keys can be easily spoofed in transactions">
  Due to the architecture of Solana transactions, reference keys can be easily spoofed inside of
  transactions. Just because your reference key was located in a confirmed transaction, does **NOT**
  mean the transaction is what you expect it to be.
</Callout>
