...
type UseGetRecentPrioritizationFeesInput = Simplify<
  {
    addresses: Parameters<GetRecentPrioritizationFeesApi["getRecentPrioritizationFees"]>[0];
  } & Pick<GillUseRpcHook<{}>, "options" | "abortSignal">
>;
type UseGetRecentPrioritizationFeesResponse = ReturnType<GetRecentPrioritizationFeesApi["getRecentPrioritizationFees"]>;
 ...
 */
export function useGetRecentPrioritizationFees({
  options,
  abortSignal,
  addresses,
}: UseGetRecentPrioritizationFeesInput) {
  ...
  return {
    ...rest,
    fees: data as UseGetRecentPrioritizationFeesResponse,
  };
}
